...
import { Field, CheckboxField, ReadOnlyField, SaveButton } from "./form-fields";
...
export function FaqForm({
  ...
  faq?: Faq;
  /** Set for operators: the FAQ always targets their own venue page. */
  operatorVenue?: { name: string };
}) {
  return (
    <form action={action} className="space-y-6 max-w-3xl">
      <div className="grid grid-cols-2 gap-4">
        ...
        {operatorVenue ? (
          <ReadOnlyField label="Shown on" value={`${operatorVenue.name} venue page`} />
        ) : (
          <>
            <SelectField
              label="Segment"
